...
1.  **Dispatcher (`dispatcher.md`)**:
    *   **Role:** Central entry point. Assesses task complexity.
    *   **Use case:** Start here. Solves simple tasks directly; launches the multi-agent system for complex ones.
    *   **Philosophy:** Efficient Dispatch & Orchestration.

2.  **Methodologist (`methodologist.md`)**:
...
3.  **Dialectician (`dialectician.md`)**:
...
5.  **Reflexive Manager (`reflexive_manager.md`)**:
...
Interact with the agents:
    *   `@dispatcher Fix this bug.` (Starts the assessment process)
